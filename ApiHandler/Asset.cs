    ...
    public static class Asset
    {
        ...
        public static ParaObjects.Asset Schema(ParaCredentials ParaCredentials)
        {
            ...
            if (ar.HasException == false)
            {
                ...
                Asset = ParaEntityParser.EntityFill<ParaObjects.Asset>(purgedSchema);
            }
        ...
        private static ParaEntityList<ParaObjects.Asset> FillList(ParaCredentials ParaCredentials, AssetQuery Query, ParaEnums.RequestDepth RequestDepth)
        {
            ...
            {
                OptimizationResult rslt = ApiUtils.OptimizeObjectPageSize(AssetsList, Query, ParaCredentials, ParaEnums.ParatureModule.Asset);
                ar = rslt.apiResponse;
            ...
            if (Query.RetrieveAllRecords && !ar.HasException)
            {
                ...
                if (Query.OptimizeCalls)
                {
                    ...
                    for (int i = 2; i <= callsRequired; i++)
                    {
                        ...
                        instance = new ThreadPool.ObjectList(ParaCredentials, ParaEnums.ParatureModule.Asset, Query.BuildQueryArguments());
